...
public sealed class FireboltCommand : DbCommand
{
    ...
    public QueryResult Execute(string commandText)
    {
        ...
        try
        {
            ...
            if (Parameters.Any()) newCommandText = GetParamQuery(commandText);
            Response = Connection?.Client
                .ExecuteQuery(engineUrl, Connection.Database, newCommandText, Connection.GetAccessToken()).GetAwaiter()
                .GetResult();
    ...
    public string GetParamQuery(string commandText)
    {
        ...
        try
        {
            foreach (var item in Parameters.ToList())
            {
                ...
                var pattern = string.Format(@"\{0}\b", item.ParameterName);
                var regexOptions = RegexOptions.IgnoreCase;
                var verifyParameters = item.Value;
                if (item.Value is string & (item.Value != null))
                {
                    var sourceText = item.Value.ToString();
                    foreach (var item1 in escape_chars) sourceText = sourceText.Replace(item1.Key, item1.Value);
                    verifyParameters = "'" + sourceText + "'";
                }
                else if (item.Value is DateTime)
                {
                    var dt = (DateTime)item.Value;
                    var date_str = dt.ToString("yyyy-MM-dd HH:mm:ss");
    ...
    {
        if (response == null) throw new FireboltException("JSON data is missing");
        var prettyJson = JToken.Parse(response).ToString(Formatting.Indented);
        var data = JsonConvert.DeserializeObject<QueryResult>(prettyJson);
