...
public class FireboltClient2 : FireboltClient
{
    ...
    private static readonly IDictionary<string, string> systemEngineUrlCache =
        new ConcurrentDictionary<string, string>();

    private static readonly string DB_QUERY = "SELECT * FROM information_schema.{0}s WHERE {0}_name=@Name";
    private readonly string _account;
    private readonly string _connectionId;

    private readonly ISet<string> _engineStatusesRunning =
        new HashSet<string>(StringComparer.InvariantCultureIgnoreCase) { "Running", "ENGINE_STATE_RUNNING" };

    public FireboltClient2(FireboltConnection connection, string id, string secret, string endpoint, string? env,
        string account, HttpClient httpClient) : base(connection, id, secret, endpoint, env, "2.0", httpClient)
    {
        _account = account;
    ...
    /// </summary>
    protected override async Task<HttpRequestMessage> GetHttpRequest(HttpMethod method, string uri,
        HttpContent? content, bool needsAccessToken)
    {
        ...
        try
        {
            ...
            var connectionInfo = new StringBuilder($" connId:{_connectionId}");
    ...
    public override async Task<string> EstablishConnection(bool forceTokenRefresh = false)
    {
        ...
        if (!forceTokenRefresh)
        {
            ...
            if (loginResponse == null)
            {
                ...
                if (loginResponse != null) connectionCache.SetCachedToken(loginResponse);
            }
        }
    ...
    protected override Task<LoginResponse> Login(string id, string secret, string env)
    {
        ...
        return SendAsync<LoginResponse>(HttpMethod.Post, $"https://id.{env}.firebolt.io{AUTH_SERVICE_ACCOUNT_URL}",
            credentials.GetFormUrlEncodedContent(), false, false, CancellationToken.None);
    }
    ...
    private async Task<T> GetAccountField<T>(string fieldName, string accountName)
    {
        ...
        try
        {
            return await GetJsonResponseAsync<T>(HttpMethod.Get, url, null, true, CancellationToken.None);
        }
        catch (FireboltException e) when (e.StatusCode == HttpStatusCode.NotFound)
        {
            throw new FireboltException(
                    ...
                    "Please verify the account name and make sure your service account has the correct RBAC permissions and is linked to a user.",
                    e)
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (systemEngineUrl != null)
        {
            ...
            if (urlParts.Length > 1)
                ProcessParameters(new FireboltConnectionStringBuilder(_connection.ConnectionString),
                    ExtractParameters(urlParts[1]), false);
        }
    ...
    private async Task<ConnectionResponse> ConnectToSystemEngine(string database)
    {
        ...
        if (_connection.InfraVersion == 2) _protocolVersion = PROTOCOL_VERSION;
    ...
    private async Task<ConnectionResponse> ConnectToCustomEngineUsingInformationSchema(string engineName,
        string database)
    {
        if (database == string.Empty)
            // If no db provided - try to fetch it
            database = await GetEngineDatabase(engineName) ??
                       throw new FireboltException(
                           $"Engine {engineName} is attached to a database current user can not access");
        var dbTerm = await GetDatabaseTable();
        var hasAccess = await IsDatabaseAccessible(dbTerm, database);
        if (!hasAccess)
            throw new FireboltException(
                $"Database {database} does not exist or current user does not have access to it!");
        ...
        var reader = await Query(string.Format(query, dbTerm), "@EngineName", engineName);
        if (!await reader.ReadAsync()) throw new FireboltException($"Engine {engineName} not found.");
    ...
    private async Task<ConnectionResponse> ConnectToCustomEngineUsingResponseHeaders(string engineName, string database)
    {
        if (!IsConnectionCachingEnabled())
        {
            ...
            if (!string.IsNullOrEmpty(database)) await Execute($"USE DATABASE \"{database}\"");
    ...
    /// <summary>
    ///     Once we verify a database with the backend we will cache its result and would consider the database valid in
    ///     between connection establishing.
    ///     If the database was deleted between the time a connection cached the database and the second connection using the
    ///     cached value, then this would only
    ...
    private async Task GetAndSetDatabaseProperties(string databaseName, ConnectionCache connectionCache)
    {
        ...
        lock (connectionCache)
        {
            ...
            if (connectionCache.IsDatabaseValidated(databaseName)) return;
        }
    ...
    /// <summary>
    ///     Once we verify an engine with the backend we will cache its result and would consider the engine valid in between
    ///     connection establishing.
    ///     If the engine was deleted between the time a connection cached the engine and the second connection using the
    ///     cached value, then this would only
    ///     be caught while executing the statement, not during connection time.
    /// </summary>
    ...
    private async Task GetAndSetEngineProperties(string engineName, ConnectionCache connectionCache)
    {
        ...
        lock (connectionCache)
        {
            connectionCache.SetEngineOptions(engineName,
                new EngineOptions(_connection.EngineUrl ?? string.Empty, engineProperties));
        }
    }
    ...
    {
        var reader = await Query("SELECT attached_to FROM information_schema.engines WHERE engine_name=@EngineName",
    ...
    {
        return await (await Query(string.Format(DB_QUERY, "table"), "@Name", "catalogs")).ReadAsync()
            ? "catalog"
            : "database";
    }
